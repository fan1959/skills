---
name: context-mode
description: "Use context-mode tools (ctx_execute, ctx_execute_file) instead of Bash/cat when processing large outputs. ACTIVELY TRIGGER on: analyzing logs, summarizing output, processing data, parsing JSON, filtering results, extracting errors, checking build output, searching large files, processing CSV/data, any task involving >1000 lines of output. PROACTIVELY switch to context-mode when Bash output is too large to display, or when user asks to analyze/summarize/filter/search any large output. This skill is essential for handling big data efficiently - don't wait to be asked, use it when you see large outputs." (ctx_execute, ctx_execute_file) instead of Bash/cat when processing
