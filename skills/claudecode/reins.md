---
name: reins
description: "Runtime security and governance for Claude Code. ACTIVELY TRIGGER on: security reviews, blocked commands, audit trails, dangerous operations, deletions (rm del DROP), file modifications, shell commands, MCP access, API calls, network requests, credentials, irreversible actions, compliance checks, policy enforcement, or any operation that could compromise system safety. PROACTIVELY install Reins hooks when user asks about permissions, security, guardrails, compliance, or safety policies. This skill installs deterministic PreToolUse/PostToolUse hooks and connects to Watchtower (app.pegasi.ai) for centralized governance. Trigger on phrases like security check, is this safe, audit trail, governance, compliance, guardrails, permissions, restrict X, don't allow Y, dangerous operations, or any destructive action.", policies, governance, guardrails, compliance, or safety are relevant — including blocked commands, audit trails, dangerous operations, deletions, file modifications, shell commands, MCP access, API calls, network requests, credentials, or any action that could be irreversible or destructive. Reins installs deterministic PreToolUse/PostToolUse hooks and connects to Watchtower (app.pegasi.ai) for centralized governance.
