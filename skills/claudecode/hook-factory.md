---
name: hook-factory
description: "Generate production-ready Claude Code hooks with interactive Q&A, automated installation, and enhanced validation. ACTIVELY TRIGGER when: user mentions hooks, before/after, triggers, automation, run before X, check for Y after Z, workflow automation, automated validation, linters, formatters, or wants to customize Claude's behavior. PROACTIVELY suggest hooks when the user does repetitive validation (e.g. every time I commit check X, before running tests verify Y). Supports 10 templates across 7 event types. Trigger on phrases like make Claude do X automatically, hook into X event, automate workflow with hooks, before/after hooks, custom trigger, Claude Code hooks, install hook." with interactive Q&A, automated installation, and enhanced validation. Supports 10 templates across 7 event types for comprehensive workflow automation.
version: 2.0.0
